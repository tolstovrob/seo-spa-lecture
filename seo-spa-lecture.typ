...
В рамках обзорной лекции мы, конечно, не будем углубляться более подробно в кодовю реализацию (тем более никто не сможет объяснить о том как устроен SSR в Next.js лучше, чем официальная документация), приведём лишь несколько примеров.
...
== Теория
SSR -- очень мощное решение для динамических и часто ваимодействующих с сервером приложений. Однако, если контент на странице меняется не так часто (например, блоги или лендинги), то SSR может стать ненужным оверинжинирингом.
